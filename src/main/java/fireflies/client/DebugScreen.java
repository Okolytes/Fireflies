...
public class DebugScreen extends Screen { // https://github.com/codingminecraft/IntegratingImGui
    ...
    public static void onKeyInput(InputEvent.KeyInputEvent event) {
        if (event.getKey() == GLFW.GLFW_KEY_RIGHT_CONTROL && event.getAction() == GLFW.GLFW_PRESS) {
            ...
            if (mc.world != null) {
                mc.displayGuiScreen(Objects.equals(mc.currentScreen, DebugScreen.getInstance()) ? null : getInstance());
            }
        }
    }
    ...
    public void render(MatrixStack pMatrixStack, int pMouseX, int pMouseY, float pPartialTicks) {
        ...
        try {
            ...
            if (selectedFirefly != null && selectedFirefly.isAddedToWorld()) {
                ...
                textValue("abdomenParticle == null", selectedFirefly.particleManager.abdomenParticle == null);
                if (ImGui.isItemHovered()) {
                    ImGui.setTooltip(selectedFirefly.particleManager.abdomenParticle.toString());
                }
                ...
                if (ImGui.treeNode("Dust particles properties")) {
                    ImGui.sliderFloat2("min/max scale", FireflyDustParticle.SCALE, 0, 0.1f);
                    ImGui.sliderInt2("min/max age (ticks)", FireflyDustParticle.AGE, 0, 200);
                    ImGui.sliderFloat2("min/max rotation speed", FireflyDustParticle.ROT_SPEED, 0, 0.1f);
                    ImGui.sliderFloat("y motion", FireflyDustParticle.MOTION_Y, 0, 0.1f);
